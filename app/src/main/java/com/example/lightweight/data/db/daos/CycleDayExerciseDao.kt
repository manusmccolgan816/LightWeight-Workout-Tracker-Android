...
interface CycleDayExerciseDao {
    ...
    @Query(
        "UPDATE CYCLE_DAY_EXERCISE " +
                "SET cycle_day_exercise_number = cycle_day_exercise_number - 1 " +
                "WHERE cycle_day_category_ID = :cycleDayCategoryID " +
                "AND cycle_day_exercise_number > :cycleDayExerciseNumber"
    )
    ...
    @Query(
        "SELECT CDE.cycle_day_exercise_ID, CDE.cycle_day_exercise_number, E.exercise_name, " +
    ...
    @Query(
        "SELECT COUNT(cycle_day_exercise_number)" +
    ...
    @Query(
        "SELECT CDE.cycle_day_exercise_ID, E.exercise_name, CDC.cycle_day_category_ID " +
    ...
    @Query("SELECT * FROM CYCLE_DAY_EXERCISE WHERE cycle_day_exercise_ID = :cycleDayExerciseID")
